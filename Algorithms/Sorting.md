...
> 데이터의 개수가 N개, 그 중 최댓값이 K일 때 복잡도 `O(N+K)` 를 보장한다.

## 정렬 방식

- <span style="color:orange">굉장히 간단한 정렬 방식을 갖고 있다</span>.
- 데이터의 범위를 알고 있을 때, 해당 범위만큼 배열을 만들어 두고 각 인덱스에 부합하는 숫자의 값을 1개씩 증가시킨다.
    - 그렇기에, 1) <span style="color:orange">***데이터의 범위를 알고 있***</span>어야 하고 2) <span style="color:orange">***정수 값 (또는 정수로 변환 가능한)을 갖고 있***</span>어야 함
...
| --- | --- | --- | --- | --- |
| 1 | 1 | 2 | 1 | 1 |
- 이제 최종 결과를 순서대로 출력하면, 오름차순 정렬 완성이다.
...
- **장점**
    - 데이터의 크기가 한정되어 있고, 데이터 중복이 있을수록 유리하게 사용할 수 있음
    - 조건만 만족한다면 <span style="color:orange">*매우 낮은 계산 복잡도*</span>로 해결할 수 있음
- **단점**
    - <span style="color:orange">*메모리의 심각한 비효율성*</span>
        - 만약, 데이터가 0과 999,999만 존재한다고 하면 배열의 크기는 여전히 1,000,000이 되어야 함
        - 데이터의 특성을 찾기 어렵다면, 퀵 정렬이 답이다 ^_^